-- Phoenix Cheat Menu — Tabbed UI (Tím & Đen) - Improved Version
...
-- Cải tiến: UI đẹp hơn (thêm border, highlight tab active, dropdown cho color thay vì textbox, slider value realtime), fix lỗi (thêm FOV check chính xác cho Aimbot, xử lý config load an toàn), thêm hình tròn 2D cho FOV Aimbot (hiển thị khi enable Aimbot)
...
local function createGui()
    ...
    Main.BorderColor3 = Config.Theme.Accent
    Main.Active = true
    Main.Draggable = true
    ...
    local function newTabButton(text)
        ...
        local function refreshHighlight()
            btn.BackgroundColor3 = (currentTab == text) and Config.Theme.Accent or Color3.fromRGB(26,20,34)
        end
        refreshHighlight()
        btn.MouseButton1Click:Connect(function()
            ...
            for _, child in ipairs(TabsPanel:GetChildren()) do
                if child:IsA("TextButton") then
                    child.BackgroundColor3 = (child.Text == text) and Config.Theme.Accent or Color3.fromRGB(26,20,34)
                end
            end
        end)
    ...
    local function slideTo(pageName)
        ...
        currentTab = pageName
        espRefresh(); aimRefresh(); moveRefresh(); tpRefresh(); setRefresh()
    end
    ...
    local function makeDropdown(parent, labelText, options, initial, onSelect)
        ...
        Instance.new("UIListLayout", listFrame).Padding = UDim.new(0,0)
    ...
    do
        ...
        colorDropdown.Position = UDim2.new(0,0,0,y); y = y + 54
    ...
    do
        ...
        showFOVToggle.Position = UDim2.new(0,0,0,y); y = y + 44
    end
...
-- ========== CORE LOGIC (ESP / Aimbot / Movement / Teleport) ==========
...
-- getClosestTarget (used by aimbot & teleport) - Fixed with proper FOV check
...
RunService.RenderStepped:Connect(function(delta)
    ...
    if Config.Movement.FlyEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        local moveVec = Vector3.new((right-left), vert, (back-forward) * -1)  -- Fixed forward/back direction
